...
const CATEGORY_COLORS: Record<ResourceCategory, { bg: string; text: string; border: string }> = {
  grief_counselor: { bg: "bg-violet-50", text: "text-violet-700", border: "border-violet-200" },
  estate_attorney: { bg: "bg-blue-50", text: "text-blue-700", border: "border-blue-200" },
  estate_cpa: { bg: "bg-green-50", text: "text-green-700", border: "border-green-200" },
  estate_sale: { bg: "bg-amber-50", text: "text-amber-700", border: "border-amber-200" },
  donation_pickup: { bg: "bg-rose-50", text: "text-rose-700", border: "border-rose-200" },
  junk_removal: { bg: "bg-stone-50", text: "text-stone-700", border: "border-stone-200" },
  crisis: { bg: "bg-red-50", text: "text-red-700", border: "border-red-200" },
};
...
export function ChatResourceCard({ resource, onMoreOptions }: ChatResourceCardProps) {
  ...
  return (
    <div className={`my-2 bg-white border ${colors.border} rounded-xl overflow-hidden shadow-sm`}>
      {/* Header */}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <h4 className="font-medium text-stone-900">{resource.name}</h4>
            ...
            {/* Description */}
            <p className="text-sm text-stone-500 mt-1">{truncatedDescription}</p>
      ...
      <div className="px-3 py-2 border-t border-stone-100 flex flex-wrap gap-2">
        {resource.website && (
          <a
            ...
            className={`flex-1 min-w-[100px] px-4 py-2.5 text-sm font-medium ${colors.text} ${colors.bg} border ${colors.border} rounded-lg hover:opacity-80 transition-opacity text-center`}
          >
            Visit website
          </a>
        )}
        {resource.phone && (
          <a
            ...
            className="px-4 py-2.5 text-sm font-medium text-stone-700 bg-stone-50 border border-stone-200 rounded-lg hover:bg-stone-100 transition-colors"
          >
            Call {resource.phone}
          </a>
        )}
        {onMoreOptions && (
          <button
            ...
            className="px-4 py-2.5 text-sm text-stone-500 border border-stone-200 rounded-lg hover:bg-stone-50 transition-colors"
          >
            More options
          </button>
        )}
...
export function ResourceCategorySelector({ onSelect }: ResourceCategorySelectorProps) {
  ...
  return (
    <div className="my-2 bg-white border border-stone-200 rounded-xl overflow-hidden shadow-sm">
      <div className="px-4 py-3 border-b border-stone-100">
        <h4 className="font-medium text-stone-900">What kind of help are you looking for?</h4>
      </div>
        ...
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              className="w-full p-3 text-left rounded-lg hover:bg-stone-50 transition-colors flex items-center gap-3"
            >
              <div className={`w-8 h-8 rounded-lg ${colors.bg} flex items-center justify-center flex-shrink-0`}>
                <div className={`w-3 h-3 rounded-full ${colors.text.replace('text-', 'bg-')}`} />
              </div>
              <div>
                <div className="font-medium text-stone-900 text-sm">{cat.label}</div>
